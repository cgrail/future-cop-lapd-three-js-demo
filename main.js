...
============================================================ */
const ARENA = { hw: 80, hd: 130 };           // half width (x), half depth (z)
...
function makeTurretModel(palette) {
  const g = new THREE.Group();
  const body = new THREE.MeshStandardMaterial({ color: palette.body, roughness: 0.55, metalness: 0.35 });
...
function makeBaseModel(palette) {
  const g = new THREE.Group();
...
const BLUE = { body: 0x2b4fd8, accent: 0x6fd2ff };
const RED = { body: 0xa42a20, accent: 0xffb03a };

/* ============================================================
   Entities
============================================================ */
const entities = [];        // everything with hp
const projectiles = [];
const particles = [];
...
function makeBaseEntity(team, z) {
...
function makeTurretEntity(team, x, z) {
  const palette = team === 'blue' ? BLUE : RED;
